			...
			<table id="user_table" class="mdl-data-table mdl-js-data-table data-table" cellspacing="0" width="100%">
			  <thead>
			    <tr>
					...
					<th class="mdl-data-table__cell--non-numeric no-sort">Descripción</th>
			  ...
			  <tbody>
			        @foreach ($rooms as $a_room)
						<tr>
							<td class="mdl-data-table__cell--non-numeric">{{ucfirst($a_room->name)}}</td>
...
@section('template_scripts')
	...
	<script type="text/javascript">
		@foreach ($rooms as $a_room)
			mdl_dialog('.dialog-trigger{{$a_room->id}}','','#dialog_delete');
		@endforeach
